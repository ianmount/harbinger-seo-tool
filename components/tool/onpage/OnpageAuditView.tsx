...
/**
 * Renderer for an on-page audit report. Pulled out of the live tool
 * page so the saved-artifact viewer can reuse the exact same UI, fed
 * by the JSON the SaveToPartnerButton captured at run time.
 *
 * Pass the `AuditReport` produced by `/api/tools/technical/onpage`
 * (or a saved copy thereof). The component is self-contained — it
 * renders the health gauge, KPI grid, category buckets, top issues,
 * per-URL table, Core Web Vitals, and schema rollup.
 */
export function OnpageAuditView({ data }: { data: AuditReport }) {
  return (
...
function SectionCard({
  ...
  return (
    <div
      className={cn("rounded-lg border border-line bg-card p-5", className)}
    >
      {title ? (

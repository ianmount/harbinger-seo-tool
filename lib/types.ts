...
/**
 * One row of the content-carryover analysis. Emitted for every URL crawled
 * from the current site. Pages that cleared the GSC traffic floor get a
 * Claude-authored recommendation grounded in title/headings/word-count plus
 * GSC clicks/impressions/position; pages that didn't clear the floor get a
 * deterministic "retire" recommendation generated server-side (no Claude
 * tokens spent on dead URLs).
 *
 * `targetPagePath` references the new sitemap by breadcrumb path (matches
 * `KeywordMapping.pagePath`) when the carried-over content has a destination;
 * null when the page is being retired or rolled into a parent.
 */
export type CarryoverRecommendation =
  | "port-as-is"
  | "port-and-refresh"
  | "rewrite"
  | "retire"

export interface ContentCarryover {
  /** Original URL from the current-site crawl (absolute). */
  oldUrl: string
  /** Recommended action for this page's body content. */
  recommendation: CarryoverRecommendation
  /** New sitemap path the content should land on. Null for `retire`. */
  targetPagePath: string | null
  /** Resolved new URL from `targetPagePath`. Null for `retire`. */
  newUrl: string | null
  /** One-sentence justification citing crawl metadata + GSC data. */
  rationale: string
  /** Page title from the crawl. Empty string if not present on the page. */
  title: string
  wordCount: number
  /** GSC traffic over the lookback window. Null when GSC isn't configured. */
  gsc: {
    clicks: number
    impressions: number
    /** Click-through rate as a fraction (0–1). */
    ctr: number
    /** Average ranking position (1.0 = #1). */
    position: number
  } | null
  /**
   * True when this row was filled in by the deterministic fallback rather
   * than Claude — i.e. the page didn't clear the GSC traffic floor and was
   * auto-flagged for retirement. The XLSX surfaces these in a different fill.
   */
 ...
 * sheets correspond 1-1 to these arrays.
 */
export interface InitialStrategyOutput {
  ...
  contentCarryover: ContentCarryover[]
  ...
  /**
   * Number of crawled URLs that cleared the GSC traffic floor and were sent
   * to Claude for content-carryover analysis. Equals `crawledUrlCount` when
   * GSC is unavailable (in that case all crawled pages are sent to Claude
   * with `gsc: null`).
   */
  carryoverAnalyzedCount: number
  /**
   * True when GSC search analytics could be pulled for `partner.website`
   * over the lookback window. False when no refresh token is configured,
   * the site isn't verified in GSC, or the API call failed — in which case
   * the carryover analysis runs metadata-only.
   */
  gscEnabled: boolean
  /** Non-fatal GSC issue for the UI banner ("not verified", "no rows", etc.). */
  gscNotice: string | null
  /** Lookback window used for GSC, in days. */

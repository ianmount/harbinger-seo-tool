  ...
  try {
    return await fetch(url, {
      ...
      headers: {
        ...
        // Browser-typical request headers. Node's native fetch otherwise
        // sends nothing beyond UA + Host, which Cloudflare-style WAFs
        // pattern-match as bot traffic and respond to with stripped 200
        // challenge bodies (no real <head>). Mirroring what real mobile
        // Chrome sends — including Sec-Fetch-* and Sec-CH-UA-* — gets us
        // past basic bot fight rules without much fuss. We can't fix the
        // TLS/HTTP2 fingerprint from Node, so determined bot detection
        // (Cloudflare "I'm Under Attack", DataDome, PerimeterX) will
        // still block us.
        Accept:
